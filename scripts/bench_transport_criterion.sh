...
run_cargo_bench() {
    ...
    if [[ -n "$BENCH_PIN_PREFIX" ]]; then
        ...
        TRANSPORT_BENCH_PROFILE="$profile" "${pin_parts[@]}" cargo bench --features benchmark-owned --bench transport_criterion -- $criterion_args "$@"
    else
        TRANSPORT_BENCH_PROFILE="$profile" cargo bench --features benchmark-owned --bench transport_criterion -- $criterion_args "$@"
    fi
}
...
The core matrix uses static allocation of 128 KiB. The camera matrix uses static allocation of 16 MiB. Stale iceoryx2 shared-memory services or samples from interrupted prior runs can affect local smoke runs; rerun after host cleanup if service creation reports stale-state conflicts.
SUMMARY
}
